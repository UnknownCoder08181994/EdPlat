/* ============================================
   VisionCardReveal — Extension Methods
   Content animation, typewriter, and reset logic.
   Loaded after vision-ripple.js (core class).
   ============================================ */

VisionCardReveal.prototype.animateContentIn = function() {
...
VisionCardReveal.prototype.startTypewriter = function() {
    // Subtitle now fades in via CSS with the heading — no typewriter needed
};

VisionCardReveal.prototype.revealCards = function() {
    this.rematerializeBackdrop();
};

VisionCardReveal.prototype.resetPage2 = function() {
    ...
    // Restore vision overlay (was hidden after wave)
    if (section) {
        const overlay = section.querySelector('.vision-overlay');
        if (overlay) overlay.style.display = '';
    }

    // Reset vision video to beginning so it replays
    if (this.vid) {
