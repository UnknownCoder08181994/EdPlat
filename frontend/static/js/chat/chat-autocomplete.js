/* Autocomplete, typing indicator, and cursor sync for AgentChat */

AgentChat.prototype.showTyping = function() {
    const msg = document.createElement('div');
    msg.className = 'chat-msg agent-msg typing-msg';
    msg.innerHTML = `
...
AgentChat.prototype.removeTyping = function(el) {
    if (el && el.parentNode) el.remove();
};

AgentChat.prototype.updateAutocomplete = async function() {
...
AgentChat.prototype.renderAutocomplete = function(suggestions) {
    this.autocompleteItems = suggestions;
    this.autocompleteIndex = -1;

    this.autocompleteEl.innerHTML = suggestions.map((s, i) => `
...
AgentChat.prototype.hideAutocomplete = function() {
    this.autocompleteEl.classList.remove('visible');
    this.autocompleteIndex = -1;
    this.autocompleteItems = [];
};

AgentChat.prototype.navigateAutocomplete = function(dir) {
...
AgentChat.prototype.selectAutocompleteItem = function(index) {
    const suggestion = this.autocompleteItems[index];
